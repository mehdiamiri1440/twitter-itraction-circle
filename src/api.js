...
async function getTimelinePage(screen_name, page, max_id = null) {
	...
	const res = await global.TwitterClient.get("statuses/user_timeline", params);
	return res;
}
...
async function getLikedPage(screen_name, page, max_id = null) {
	...
	const res = await global.TwitterClient.get("favorites/list", params);
	return res;
}
...
async function getAvatars(ids) {
	...
	const res = await global.TwitterClient.get("users/lookup", params);
...
async function getUser(screen_name) {
	...
	const res = (await global.TwitterClient.get("users/lookup", params))[0];
